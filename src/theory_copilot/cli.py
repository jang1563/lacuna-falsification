...
def _cmd_compare(args: argparse.Namespace) -> int:
    ...
    print()
    print("Run the PySR sweep manually (edit --genes for your cohort):")
    print(
        f"  python3 src/pysr_sweep.py --data {local_path} "
        f"--genes CA9,VEGFA,LDHA,AGXT,ALB,NDUFA4L2,SLC2A1,ENO2 "
    ...
    print(
        ...
        f"--data {local_path} --standardize --covariate-cols age,batch_index "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_compare.add_argument("--config", required=True, help="Path to datasets.json")
    p_compare.add_argument("--proposals", required=True, help="Path to law_proposals.json")
    p_compare.add_argument(
        "--flagship-dataset", required=True, help="Dataset ID for the flagship run."
    )
