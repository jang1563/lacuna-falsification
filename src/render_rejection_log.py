...
<div class="filters">
  <div class="filter-group">
    <span>Search</span>
    <input type="text" id="filterBox" placeholder="equation, gene, reason…" style="padding:4px 8px; width:260px; border:1px solid #ccc; border-radius:4px;">
  </div>
  <div class="filter-group">
    <span>Cohort</span>
    <select id="cohortSel"><option value="">All</option></select>
  </div>
  <div class="filter-group">
    <span>Task</span>
    <select id="taskSel"><option value="">All</option></select>
  </div>
  <div class="filter-group">
    <span>Panel</span>
    <select id="panelSel"><option value="">All</option></select>
  </div>
  <div class="filter-group">
    <span>Verdict</span>
  ...
  <label style="margin-left:8px; color:#666; font-size:0.85em;" id="matchCount"></label>
</div>
<table>
<thead>
<tr>
...
<script>
  // Populate dropdowns dynamically from data attributes
  ...
  populateSelect("cohortSel", "cohort");
  populateSelect("taskSel", "task");
  populateSelect("panelSel", "panel");

  const box       = document.getElementById("filterBox");
  const cohortSel = document.getElementById("cohortSel");
  ...
  function applyFilter() {{
    ...
    document.querySelectorAll("tbody tr").forEach(tr => {{
      ...
      const ok =
        ...
        (!task   || tr.dataset.task   === task)   &&
        (!panel  || tr.dataset.panel  === panel)  &&
        (!verdict || (verdict === "pass" ? passes : !passes));
      tr.style.display = ok ? "" : "none";
      if (ok) shown++;
    }});
    matchCount.textContent = shown + " rows";
  }}

  [box, cohortSel, taskSel, panelSel, verdictSel].forEach(el =>
    el.addEventListener(el.tagName === "SELECT" ? "change" : "input", applyFilter)
  );
  applyFilter(); // show initial count
</script>
</body>
</html>
"""
