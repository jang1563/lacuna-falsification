...
**Cut to browser showing PhL-8d `claude.ai/code/session_01CgsJYAPdvhJJwTuBt7QZLZ` URL
...
`3af3a67`, lock SHA `88eaca3`; then pane H —
`results/external_validation_ipf/run_manifest.json` with
`engine_caught_advocate_fabrications` showing the two empirically false
Advocate claims and the Skeptic-caught reality checks.**
...
- **Browser tab** pre-loaded to PhL-8d `claude.ai/code/session_01CgsJYAPdvhJJwTuBt7QZLZ`
  in an incognito window so the Routine-session screenshot is clean.
...
- **Pane H (NEW for IPF)**: editor open at
  `results/external_validation_ipf/run_manifest.json` with
  `engine_caught_advocate_fabrications` scrolled into view.
