...
**What this is in one sentence:** A pre-registered deterministic gate (plain Python, 5 statistical tests) rejected 194 of 203 candidate evaluations on TCGA-KIRC; the 9 survivors all came from the 45-gene metastasis_expanded sub-layer (the original 11-gene layer rejected 100% before the loop diagnosed panel absence and repaired only that cause), and the system's own downstream extension was then rejected by a separately pre-registered survival gate on independent clinical trial data (IMmotion150, n=263). The two live Routines sessions below are recorded proof that the falsification loop runs as code, not just as a demo (live view requires Claude account login; full event logs retained as static artifacts).
...
| **`make` targets** | `Makefile` | `make venv`, `make smoke` (~1 min fast judge-visible confidence check on this laptop), `make test` (115 local-runnable tests in the current target; 107/107 at submission time, +8 post-hackathon for Lacuna-Bench/Failure-Atlas/RL-readiness), `make audit`, `make h1`, `make h2`, `make prereg-audit`, `make rejection-log`, `make skeptic-review` |
