...
| **Path C — Dual Verdict Oracle (PhL-8d)** | Same oracle fires with **two equations** in one trigger: Eq1 (HIF-axis textbook law on tumor_vs_normal, expected FAIL) + Eq2 (CDK1−EPAS1 on metastasis_expanded, expected PASS). One session URL = full Lacuna narrative: pre-registered gate rejects AND accepts, same thresholds, same ~6 min autonomous run. **Pending fire — run `src/phl8d_dual_verdict_fire.py`.** | Eq1: FAIL (delta_baseline≈+0.019 < 0.05); Eq2: PASS (delta_baseline≈+0.062) — **PENDING** | [`results/live_evidence/phl8d_dual_verdict/SUMMARY.md`](../results/live_evidence/phl8d_dual_verdict/SUMMARY.md) |
...
| **PhL-8d** | pending 2026-04-26 | 1 Routine API trigger (dual verdict) | pending | 2 equations: FAIL (HIF-axis, tumor_vs_normal) + PASS (CDK1−EPAS1, metastasis_expanded); same gate, same thresholds | ~6–8 min | **PENDING** — run `src/phl8d_dual_verdict_fire.py` |
